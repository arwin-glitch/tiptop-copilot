...
/**
 * Keeps an open Today tab's briefing cards current. It asks only for a cheap
 * fingerprint and refreshes the page when that differs from what was
 * rendered — never on a timer alone, because every Today render also syncs
 * the calendar and may generate an outlook. Hidden tabs do not poll; coming
 * back to one (including from the back/forward cache) checks at once. Failed
 * checks back off, and a signed-out tab stops checking.
 */
export function BriefingWatcher({ version }: { version: string }) {
  ...
  React.useEffect(() => {
    ...
    async function check() {
      ...
      inFlight = true;
      try {
        const response = await fetch('/api/briefings/current', {
          cache: 'no-store',
